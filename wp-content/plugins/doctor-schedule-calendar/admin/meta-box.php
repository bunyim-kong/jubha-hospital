...
add_action('add_meta_boxes', function () {
    add_meta_box('doctor_schedule','Doctor Weekly Availability','doctor_schedule_cb','doctor');
});
...
function doctor_schedule_cb($post){
    ...
    foreach($days as $day){
        ...
        echo "<label><input type='checkbox' name='schedule[$day][off]' ".checked($off,true,false)."> Not Available</label> | ";
...
add_action('save_post', function($post_id){
    if(isset($_POST['schedule'])) update_post_meta($post_id,'_doctor_schedule',$_POST['schedule']);
});